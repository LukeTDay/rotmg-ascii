...
"Printed directly" means bypassing curses' own character-output path entirely — **not** embedding the escape sequence inside a string passed to `window.addstr()`/`pad.addstr()`. Confirmed via `pad.instr()` read-back on this repo's `windows-curses` build: `addstr` doesn't pass the raw `0x1B` (ESC) byte through, it stores it as the two visible characters `^[`, which would render as literal garbage text (e.g. `^[[38;2;255;140;0m`) instead of an invisible color code. A real implementation needs to write the ANSI bytes straight to the terminal (e.g. `sys.stdout.write`/`os.write(1, ...)`) at an absolute cursor position, timed after the relevant `pad.refresh()` call so curses doesn't overwrite it on the next redraw — this hasn't been built yet anywhere in the app; `Debug/cp437_full_charset_16color_test.py` (next paragraph) predates it and only exercises the native-palette path, not raw ANSI.
