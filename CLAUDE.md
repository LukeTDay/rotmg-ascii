...
- **`Screen.py`** — the `Screen` enum (`accountSelect`, `enterAccountInfo`, `login`, `charSelect`, `charCreate`, `charDelete`, `serverSelect`, `gameScreen`, `exit`) that drives `main.py`'s state machine.
- **`ApiPoints.py`** — every real `realmofthemadgod.com` HTTP endpoint this app calls (`VERIFY`, `CHAR`, `CHARDELETE`, `SERVERS`, `FRIENDSLIST`, `GUILDMEMBERS`, etc.), plus the Unity `User-Agent`/`X-Unity-Version` headers needed to look like the real client.
...
- **`PlayerData.py`** — the local player's full stat block (HP/MP, ATK/DEF/SPD/DEX/VIT/WIS, inventory, guild, exaltations, potions, ...). `parse(obj)` reads an initial `CREATESUCCESS`/`UPDATE` object; `parseStats(stats)` is a big `StatTypes`-keyed if/elif chain applying incremental stat updates from `NEWTICK`. `conditionExpiry` (effect id → wall-clock expiry) is the one field not driven by server stats at all — populated/cleared by `hitDetection.checkAoeHits`/`pruneExpiredConditions` for locally-predicted AOE status effects.
...
- **`TileManager.py`** — turns `GameState` + `ProjectileStore` + `AoeStore` + `MeteorWarningStore` into what actually gets drawn each frame.
  - `buildVisibleTiles(...)` — rebuilds the visible tile window from scratch every frame (not incrementally synced): buckets every in-range object/projectile/AOE telegraph-or-instance/meteor-warning by tile, then resolves one `RenderCell` (char + color + `bold`) per tile via the render-hierarchy precedence below.
  - `classifyObject(...)` — single-pass tier classification (`_TIER_PRIORITY`, highest first): `Self > Enemy > Loot bag > Projectile > Meteor warning > Wall > Portal > Interactive NPC > Other player (friend/guild/locked only) > AOE > Floor`. Anything matching no tier is excluded entirely, not drawn as a lower one. Meteor warnings rank just under Self/Enemy/Projectile since they're a "must be seen" danger cue; AOE ranks lowest (only ever replaces bare floor) so a blast circle never hides a wall/enemy/portal/other tile.
...
- **`ProjectileStore.py`** — client-side-simulated in-flight bullets (RotMG never re-sends a projectile's position after the shot packet). `Projectile.posAt(now)` computes current position from `startingPos`/`angle`/`speed`, branching on four mutually-exclusive real `<Projectile>` XML flags (confirmed against `Resources/_generated/xml` and cross-checked formula-for-formula against 3 reference server/client implementations — see `CLAUDE.local.md`'s "Reference projects" section): plain straight-line (default), `wavy` (the firing angle itself oscillates), `parametric` (ignores speed entirely, traces a `magnitude`-scaled Lissajous curve over the shot's lifetime), and `boomerang` (distance reflects at the lifetime's temporal midpoint) — `amplitude`/`frequency` layer a perpendicular sine deflection onto the plain/boomerang branch only. `ProjectileStore.spawn`/`remove`/`prune` manage the store, keyed by `(ownerId, bulletId, shotIndex)` — `shotIndex` also drives the even/odd phase alternation between adjacent shots in a fan (wavy/amplitude weave oppositely; parametric fans into distinct curve variants). `Projectile.fromEnemy` (set at spawn - `ENEMYSHOOT` vs `SERVERPLAYERSHOOT`) and `hitTargetIds`/`hitLocalPlayer` back the two directions of client-side hit detection in `Renders/GameScreen/hitDetection.py`; `prune()` itself only removes expired bullets, hit-connected ones are removed by that module instead.
- **`AoeStore.py`** — client-side AOE-grenade rendering + local hit-prediction, sourced from `AOE` (landed, ground truth) and `SHOWEFFECT` (`THROW`/`THROW_PROJECTILE`, pre-impact telegraph) packets. `AoeStore.land(...)` is the *only* place either radius or throw-duration ever gets learned — keyed by `(origType, color)` (`origType` confirmed via live capture to be the throwing enemy's own `objectType`; color is included too since one enemy can have more than one distinct AOE ability, which would otherwise thrash a single shared radius) — and persists the confirmed tables to `Resources/aoeRadiusCache.json` (`Utils/json/aoeRadiusCacheLoader.py`, tracked/committed rather than gitignored, so accumulated knowledge survives a fresh clone). `spawnTelegraphIfKnown(...)` is `SHOWEFFECT`'s side and is strictly read-only against those tables — no telegraph (and no placeholder-radius guess) unless the key is already known from a prior landing — growing a marker from 0 toward the known radius over the confirmed/measured duration, turning bold in the final quarter of the countdown. `THROW_PROJECTILE`'s `color` field is confirmed to actually hold the flying visual projectile's own `objectType` (e.g. Yeti Boulder), not a real color — handled via the `colorHint` param, `None` meaning "fall back to matching this enemy alone." `SHOWEFFECT`'s `effectType` enum (and which ids are confirmed vs. still unverified) is documented on `Networking/Packets/Incoming/ShowEffectPacket.py`.
- **`MeteorWarningStore.py`** — `MeteorWarning`/`MeteorWarningStore`: a fixed 1-tile `!` magenta warning marker for `SHOWEFFECT`(`METEOR`), deliberately much simpler than `AoeStore` — no learned radius, no persistence, no hit-detection, just visible for the packet's own claimed duration then pruned.
...
- **`Listener.py`** — owns the socket's *read* side. `start()` loops on `recv()`, decodes each packet, and for a handful of latency-critical types replies immediately from this thread rather than waiting on the render loop: `PING`→`PONG`, `UPDATE`→`UPDATEACK`, `GOTO`→`GOTOACK`, `SERVERPLAYERSHOOT`→`SHOOTACK` (if it's this client's own shot), `ENEMYSHOOT`→`ENEMYSHOOTACK`, `AOE`→`AOEACK`, `NEWTICK`→ a `MOVE` built from `Ticker.drainRecords()`. Tracks `self.objectId` (from `CREATESUCCESS`) so it knows which events are about the local player. Forwards every decoded packet onto the incoming queue too — except `RECONNECT`, which becomes a `("connecting", mapName)` tuple instead (the listener thread can't cleanly tear itself down mid-`recv()`).
...
Lists the account's characters (sorted by fame, scrollable), colored by seasonal/crucible/standard status. `_buildEquipmentRow`/`_equipSlotText` resolve each character's first 4 equipment slots to item names via `Utils/json/objectNameLoader.objectIdToName`. Selecting a character sets `ctx["CURR_CHAR_ID"]` and moves to `Screen.serverSelect`. Extra trailing rows below the real characters (`"create"`/`"delete"`/`"back"` in `extraSlots`) route to `Screen.charCreate` (only shown when a class slot is free), `Screen.charDelete` (only shown when there's at least one character), and `Screen.accountSelect`. `printCharSlot`/`printBackSlot`/`printCreateSlot`/`printDeleteSlot` are deliberately non-underscored (unlike this file's other helpers) since `charDelete.py` imports `printCharSlot` directly to render its own list identically rather than duplicating the row layout.

### `CharCreate/charCreate.py`

Two-stage class-creation flow (`stage = "grid" | "confirm"`): a grid of every class in `Constants/ClassIds.ALL`, colored/locked per `ctx["CHARDATA"].unlockedClasses` (`_lockMessage` explains a locked class's level/precursor requirement), then a Seasonal-vs-Standard confirm screen (`_printSeasonalRow`/`_classAttr` — a reusable two-option Left/Right selector pattern, later reused as-is by `charDelete.py`'s Yes/No prompt). Confirming sets `ctx["PENDING_NEW_CHAR"]` (a `PendingNewChar` NamedTuple in `Models/Context.py`) and moves to `Screen.serverSelect` — the actual `CREATE` packet is sent later, once connected, by whatever reads that pending value. ESC/the grid's "Back" row return to `Screen.charSelect`, except when the account has zero characters (`canLeaveToCharSelect`), where it's forced to stay and create one first.

### `CharDelete/charDelete.py`

Reachable from both `charSelect.py`'s "Delete Character" slot and the pause menu's "Delete Character" entry (`Renders/PauseMenu/pauseMenu.py`). Same two-stage shape as `charCreate.py`: a scrollable character list (stage `"list"`, reusing `charSelect.printCharSlot`/its own `_printBackSlot`) plus a confirm stage showing the selected character's box and a Yes/No row (`_printYesNoRow`/`_classAttr`, mirroring `charCreate._printSeasonalRow` — red "Yes", green "No", **defaults to No**). Selecting Yes calls the real `char/delete` endpoint synchronously (`_deleteCharacter`, same `requests.post(..., params={...})` + `"Success" in r.text` check convention as `authentication/getAccessAndClientToken.py`/`login.py`) and, on success, removes the character from `ctx["CHARLIST"]` **in place** (same list object `charSelect.py` sorts/mutates, so no separate resync is needed) and from `ctx["CHARDATA"].charIds`. Choosing No, or ESC, returns to the list stage on the same character rather than resetting selection.
...
- **`mapRenderer.py`** — turns `TileManager.buildVisibleTiles`'s output into actual `pad.addstr()` calls, plus the HP/MP/Fame HUD.
  ...
  - `_drawMap` — writes each visible tile's glyph as a `scaleX × scaleY` block of characters, ORing in `curses.A_BOLD` when the tile's `RenderCell.bold` is set (AOE/meteor-warning tiles).
...
- **`inputRouter.py`** — resolves a frame's drained keys (`movementInput.drainKeys`) against `ctx["KEYBINDS"]` (`Utils/json/keybindLoader.py`), shared by `movementInput.py`/`shootInput.py`/`chatPanel.py` instead of each hardcoding its own key table. `resolveDirection` — arrow keys always work regardless of config, layered with whatever letter each `moveUp`/`moveDown`/`moveLeft`/`moveRight` action is bound to. `resolveVkByDirection` rebuilds the Windows `GetAsyncKeyState` VK table fresh every call (cheap, 4 entries) so a keybind changed mid-game via the pause menu takes effect immediately. `isAutoFireToggle`/`isChatOpenKey`/`isNexusKey` are single-letter-binding checks. `getNexusMode`/`toggleNexusMode` — a 2-value enum (`NEXUS_MODE_ESCAPE` asks the server and waits for `RECONNECT`; `NEXUS_MODE_DIRECT_CONNECT` tears down and reconnects client-side immediately, same path as the very first connect) stored alongside the letter keybinds in the same dict.
...
- **`hitDetection.py`** — client-side hit detection both ways, since RotMG expects whichever side gets hit to report it, not the server. `checkProjectileHits` sends `ENEMYHIT` for the local player's own projectiles within an UNVERIFIED estimated hitbox radius (`_enemyRadiusTiles`, 0.5 tiles times a `<CustomHitbox>` scale if the target has one) of a live enemy; `HitTracker` logs confirmed-vs-unconfirmed sends - confirmed by watching the target's own `HPSTAT` drop on a later `UPDATE`/`NEWTICK` (`checkHpDrop`), not by matching the rare `DAMAGE` packet, which real capture showed isn't a general per-hit ack. `checkPlayerHits` is the mirror for incoming shots: sends `PLAYERHIT` for any `Projectile.fromEnemy` bullet within the fixed 0.5-tile player hitbox, letting the server apply the real damage/defense math and echo the reduced `HPSTAT` back on the next `NEWTICK` - the HUD HP bar (`inventoryPanel.drawBars`) already renders straight from `player.hp`, so no separate prediction/plumbing is needed for it to update. `EnemyTargetTracker` tracks which enemy hit is "currently being fought" for `enemyHealthBar.py`'s HP bar - a non-boss hit can switch the display after a cooldown, a boss hit pins it for a few seconds. `checkAoeHits` is this app's only local (client-predicted) damage/status application — every other hit path above waits for the server's echoed stat — applying an `AoeInstance`'s damage/condition-effect bit directly to `PlayerData` the moment the player's within 0.5 tiles, gated by `AoeInstance.applied` so it only ever fires once; `pruneExpiredConditions` locally expires that condition bit again after its duration (the only locally-timed condition-effect expiry in the app — `CONDITIONSTAT` is otherwise always a wholesale server-sent replacement). Both AOE predictions get silently overwritten whenever the next authoritative `HPSTAT`/`CONDITIONSTAT` arrives.
...
- **`enemyHealthBar.py`** — `drawEnemyHealthBar` overlays a name+HP-bar strip on the map area's top-left corner for whichever enemy `EnemyTargetTracker` (`hitDetection.py`) currently points at, drawn directly on the shared pad after `drawFrame`. Clears the tracked target itself if the enemy's gone from `state.objects` (died) or its `HPSTAT` reads `<= 0`.
- **`miniMap.py`** — minimap drawn into `chatPanel`'s reserved top-left box, sized off the current map's width/height to exactly fill it (downsampling a big map, magnifying a small one). `computeMinimapLayout` picks the scale; `applyTileUpdates` incrementally folds newly-arrived `UPDATE` tiles into `ctx`'s cached `MinimapCache` (fed from `gameScreen.py` as tiles arrive, not rebuilt from scratch every frame like `TileManager.buildVisibleTiles`); `drawMiniMap` renders it, `'%'` marking a player or teleportable beacon, `'@'` the local player. Clicking a `'%'` cell doesn't teleport directly - `resolveMinimapClick`/`collectTpBuckets`/`handleMinimapInput` hand off to a candidate-list overlay (`chatPanel.py`'s bottom section) when more than one teleport target shares a cell.

### `PauseMenu/`

The in-game ESC menu. `drawPauseMenu(stdscr, ctx) -> Optional[Screen]` is **not** a routed `Screen` — it's called directly as a blocking function from `gameScreen.py`'s `_connectedLoop` since that loop has no clean way to pause/resume mid-connection. Returns `None` to resume gameplay untouched (ESC, "Resume Game"), or a `Screen` for the caller to tear the connection down (`_teardownConnection`) and leave `gameScreen` entirely (e.g. `Screen.charSelect`, `Screen.charDelete`).

- **`pauseMenu.py`** — a 3×3 `_GRID` of labels, 2D arrow/WASD navigation (`_printGridRow` — same fixed-width-column technique as `serverSelect.py`, generalized to N columns). `_activate` maps each label to either a `Screen` return value, `None` (resume), or `_STAY` (a sentinel distinct from `None` meaning "handled inline, stay in this menu" — used by "Change Keybinds" and "About / Version", which just call another blocking function and loop). `_showMessage` is a generic reusable dismiss-on-any-key message box (banner + lines), used by `_showAbout`.
- **`keybindConfig.py`** — `drawKeybindConfig`, reached via "Change Keybinds". A nested (never touches `Screen`/`main.py`) two-tab screen: GAMEPLAY (edits `ctx["KEYBINDS"]` in place, persisted immediately via `saveKeybinds`) and DEBUG (packet logging, delegates to `debugOptions.py`, edits `ctx["PACKET_LOG_SETTINGS"]` in place). Pressing Enter on a keybind row enters a single-keystroke edit mode; `_findConflict` rejects a candidate key already bound to another action instead of silently double-binding it. The `nexusMode` field is a 2-value toggle (`inputRouter.toggleNexusMode`) rather than a single keystroke, handled as a special case.
- **`debugOptions.py`** — backs the DEBUG tab: packet-log level cycling and a per-packet-type enable/disable checklist. `packetNames()` reads live off `Constants/PacketIds.idToType` (never hardcoded) so a new packet type shows up already enabled with no migration step. `activate(row, ...)` mutates `ctx["PACKET_LOG_SETTINGS"]` and calls `saveDebugSettings` immediately.
...
- **`colorUtils.py`** — `nearestCursesColor(rgbInt, avoidBlack=False)`: decodes a raw wire `0xRRGGBB` int (e.g. `AoePacket.color`) to one of curses' 8 base colors. `avoidBlack=True` does a true nearest-neighbor against the 7 non-black corners instead of the plain cube-corner threshold, so a dark/black wire color still renders visibly. A small runtime port of `Scripts/AssetPipeline/deriveRenderInfo.nearestCursesColor`, which is build-time-only and not imported by the live game loop.
...
- **`keybindLoader.py`** — `loadKeybinds()` layers `Config/Keybinds/keybinds.json` (personal overrides, if present) on top of the shipped `Resources/keybindsDefault.json`; `saveKeybinds(keybinds)` writes it back atomically (tempfile + `os.replace`, same convention as `accCredLoader`'s credential file). Backs `Renders/PauseMenu/keybindConfig.py`.
- **`debugSettingsLoader.py`** — `loadDebugSettings()`/`saveDebugSettings(level, disabledPackets)` for `Config/Debug Settings/debugSettings.json` (packet-log level + per-type disable set). No shipped default file, unlike keybinds — the packet list itself is always read live off `Constants/PacketIds.idToType`, never persisted. Backs `Renders/PauseMenu/debugOptions.py`.
- **`lastServerLoader.py`** — `loadLastServer()`/`saveLastServer(name)` for `Config/Last Server/last_server.json`, same atomic-write convention. Used by `serverSelect.py` to preselect the previously-picked server.
- **`aoeRadiusCacheLoader.py`** — `loadAoeRadiusCache()`/`saveAoeRadiusCache(radii, durations)` for `Resources/aoeRadiusCache.json`, same atomic-write convention — but unlike the other loaders above, this file lives in `Resources/` and is tracked/committed (see that section below), not gitignored personal state. Backs `Models/AoeStore.py`'s self-teaching `(origType, color)` → radius/throw-duration tables — the only loader in this app whose keys are compound tuples (encoded `"origType:color"` on disk, parsed back to `(int, int)` on load).
...
- **`renderMap.json`** — one of a handful of trackable JSON files under `Resources/` (`.gitignore` has a blanket `*.json` rule with a per-file negation for each). `objectType`/`groundType` → `{name, chars, color, blocksMovement, isEnemy, isLootBag, isPortal, isInteractiveNpc}`. Generated by the asset pipeline; read via `Utils/json/objectNameLoader.py`.
...
- **`aoeRadiusCache.json`** — self-taught `(origType, color)` → real AOE blast radius / throw-to-impact duration (see `Models/AoeStore.py`). Unlike `renderMap.json`/`projectileMap.json`, this isn't generated by the asset pipeline — it grows purely from live gameplay traffic, one entry per encountered AOE, and is committed so that knowledge accumulates across the repo rather than resetting per clone/machine.
