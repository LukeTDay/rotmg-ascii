...
def handleShootInput(keys: List[int], stdscr: curses.window, ticker: Ticker, player: PlayerData,
    ...
    - shared since curses only yields each keystroke once). Toggles auto-fire,
    tracks mouse aim, and while firing sends one PLAYERSHOOT packet per
    projectile in the weapon's fan (confirmed via MITM - not one per trigger
    pull). Spawns each shot into `projectiles` immediately (client-side
    prediction) rather than waiting on the server's echo; a later echo reuses
    the same (ownerId, bulletId, shotIndex) key, so it just refreshes the
    entry instead of double-spawning.
    """
    ...
    if sawMouseEvent:
        # getmouse() only returns the latest event regardless of how many
        # KEY_MOUSE entries were drained - call once per frame, not once per
        # queued event (REPORT_MOUSE_POSITION can queue one per pixel of motion).
        try:
